	...
	protected void run() {
		...
		if (args.length>0) {
			...
			for (int i = 0; i < factsToMake; i++){
				...
				fact = new Fact(env.getAssignments(), new ArrayList<>(roomQ));
			...
			System.out.println(bestFact.getScore());
			//fact.CalculateViolations();
			//System.out.println(fact.getScore());
...
//			Person[] d = t.get(test.remove().getRoom());
	...
	protected void printResults() {
		String output;
		output = bestFact.toString();//getEnvironment().allAssigned();
